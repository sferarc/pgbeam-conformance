...
The verdict distribution is deliberately mixed (23 block, 9 mask, 3 row-filter, 10 allow). A corpus that only blocks proves nothing about over-blocking, and one that only allows proves nothing about enforcement. A test fails if any of the three groups empties out, and another fails if these four counts stop matching the corpus.
